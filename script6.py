...
def recommend():
    try:
        ...
        # Match user input with generated rules
        matching_rules = rules[rules['antecedents'].apply(lambda x: set(user_input).issubset(x))]
        print("Matching Rules:\n", matching_rules[['antecedents', 'consequents', 'confidence']])
...
def add_data():
    if request.method == 'POST':
        ...
        tv = int(request.form['tv'])  # Add more fields as necessary
        oven = int(request.form['oven'])
        officeLight = int(request.form['officeLight'])  # Add more fields as necessary
        ...
        new_data = {
            ...
            'wardrobe': wardrobe, 
            'tv': tv,
            'oven': oven,  # Add more fields as necessary
